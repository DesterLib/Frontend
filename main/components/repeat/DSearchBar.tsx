...
const SearchCardContainer = ({ movies, serie, people}: any) => {
  const theme = useTheme();
  return (
    <Grid container>
      <Grid container spacing={1.2}>
        <Grid item xs={6}>
          <Typography sx={{display: "flex", alignItems: "center", marginBottom: "10px"}} variant="subtitle2"><i style={{paddingRight: "5px"}} className="ri-movie-2-fill"></i>Movies</Typography>
          <List sx={{ width: '100%', bgcolor: 'background.paper', borderRadius: "5px", padding: "5px" }}>
            {movieData.map((item, index) => (
            <ListItemButton sx={{borderRadius: "5px", padding: "5px"}} key={index}>
              <ListItemAvatar sx={{marginRight: "5px"}}>
                <Avatar sx={{width: "50px",height: "70px"}} variant="rounded" src={item.poster}/>
              </ListItemAvatar>
        ...
        <Grid item xs={6}>
        <Typography sx={{display: "flex", alignItems: "center", marginBottom: "10px"}} variant="subtitle2"><i style={{paddingRight: "5px"}} className="ri-tv-fill"></i>TV Serie</Typography>
        <List sx={{ width: '100%', bgcolor: 'background.paper', borderRadius: "5px", padding: "5px" }}>
