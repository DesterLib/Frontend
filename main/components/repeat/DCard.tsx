...
import { APP_API_PATH, APP_API_VERSION_PATH, APP_NO_IMAGE_POSTER, APP_POSTER_QUALITY } from '../../config';
...
const BottomButtonWrapper = styled('div')(({ theme }) => ({
    position: "absolute",
    bottom: "0",
    padding: "10px",
    display: "flex",
...
const DCard = ({ item, type }: any) => {
    ...
    const openInfoModal = () => setOpenModalState(true);
    const closeInfoModal = () => setOpenModalState(false);
        ...
        <Card>
            <CardWrapper>
                <ImageWrapper className="imageWrapper">
                    <Image 
                        className="image"
                        src={(item && item.poster_url && `${APP_API_PATH}${APP_API_VERSION_PATH}/assets/image/${APP_POSTER_QUALITY}${item.poster_url}`) || (APP_NO_IMAGE_POSTER)}
                        alt="" 
                        layout="fill"
                        objectFit="cover"
                    />
            ...
            <CardTitle>
                {(item && item.title && (item.title || null)) || item && item.name && (item.name || null)}
            </CardTitle>
